...
import {
  chamDiemSinhVien,
  getKetQuaDanhGiaDonVi,
  getMinhChungByMssv,
  getMySinhVienPhuTrach,
  getTieuChiDanhGia,
} from '../../services/donViHuongDanService';
import '../GiangVien/GiangVienDashboard.css';
...
function DonViHuongDanDashboard({ account, onLogout }) {
    ...
    async function loadCriteriaAndScores() {
      ...
      try {
        const [criteriaResult, result] = await Promise.all([
          ...
          getKetQuaDanhGiaDonVi(selectedStudent.idKiThucTap, getMssv(selectedStudent)),
        ]);
  ...
  useEffect(() => {
    ...
    for (const clo of criteria?.clos || []) {
      ...
      const value = chang === 1
        ? resultClo?.diemChang1Dvhd
        : resultClo?.diemChang2Dvhd;
  ...
  async function handleSubmitScores(event) {
    ...
    try {
      await chamDiemSinhVien({
        ...
        chang,
        nguoiChamLoai: 'DVHD',
        nguoiChamId: String(account?.idDonViHD),
        diemClo,
      });

      const result = await getKetQuaDanhGiaDonVi(
    ...
    <main className="gv-dashboard">
      <aside className="gv-sidebar">
        <div className="gv-brand">
          <div className="gv-mark">ĐV</div>
          <div>
            <span>Đơn vị hướng dẫn</span>
            <strong>{account?.tenDonViHD || account?.email || 'ĐVHD'}</strong>
          </div>
        </div>

        <div className="gv-meta">
          <span>ID ĐVHD</span>
          <strong>{account?.idDonViHD || '-'}</strong>
          <span>Sinh viên tại đơn vị</span>
              ...
              {!loading && filteredStudents.map((student) => {
                ...
                return (
                  ...
                  >
                    ...
                    <small>{student.tenGiangVien || 'Chưa phân công GVHD'}</small>
                  </button>
                );
              })}

              {!loading && filteredStudents.length === 0 && (
                <div className="gv-empty">Chưa có sinh viên tại đơn vị.</div>
              )}
            </div>
          </aside>
                    ...
                    {!criteriaLoading && (criteria?.clos || []).map((clo) => {
                      ...
                      return (
                        <tr key={clo.tenClo}>
                          <td>{clo.tenClo}</td>
                          <td>{clo.moTaClo || '-'}</td>
                          <td>{Number(clo.trongSoDonViHuongDan || 0)}%</td>
                          <td>
                            <input
                              type="number"
                              min="0"
...
export default DonViHuongDanDashboard;
