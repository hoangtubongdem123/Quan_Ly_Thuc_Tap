---
name: gpt-taste
description: Elite UX/UI & Advanced GSAP Motion Engineer. Enforces layout variance, strict AIDA page structure for full pages, wide editorial typography (bans 6-line wraps), gapless bento grids, strict GSAP ScrollTriggers (pinning, stacking, scrubbing), inline micro-images, and massive section spacing.
---
...
LLMs are inherently lazy and always pick the first layout option. To prevent this, you MUST make a brief internal selection step in your `<design_plan>` before writing any UI code.
Select internally varied options for:
- 1 Hero Architecture (from Section 3)
- 1 Typography Stack (Satoshi, Cabinet Grotesk, Outfit, or Geist. NEVER Inter)
- 3 Unique Component Architectures (from Section 6)
- 2 Advanced GSAP Paradigms (from Section 5)
Do not claim to perform exact character-count math or spatial verification. Avoid defaulting to the same UI twice, and follow the selection you have made in the plan.

## 2. AIDA STRUCTURE & SPACING
Output the code as a single React component using Tailwind CSS and Next.js (App Router) conventions, including necessary client-side directives like "use client".
...
- **The Line Limit:** Apply max-width and dynamic font scaling (like `clamp(3rem, 5vw, 5.5rem)`) optimized to typically constrain the H1 to 2 to 3 lines. 4, 5, or 6 lines is a catastrophic failure to avoid.
...
- **Image Context & Style:** Use `https://picsum.photos/seed/{keyword}/1920/1080` and match the keyword to the specific industry, theme, or core subject of the user's request. Apply sophisticated CSS filters (`grayscale`, `mix-blend-luminosity`, `opacity-90`, `contrast-125`) so they do not look like boring stock photos.
